...
const traceImages: Record<string, { src: string; label: string; position: string; fit?: "cover" | "contain" }> = {
  ...
  finger: { src: "/trace-images/finger.webp", label: "DEPTH MAP · 지두흔", position: "50% 50%", fit: "contain" },
...
function ParticleStory() {
  ...
  useEffect(() => {
    ...
    const loadPolygonModel=async()=>{
      ...
      const vertexShader=compileShader(gl,gl.VERTEX_SHADER,`attribute vec3 aPosition;attribute vec3 aNormal;uniform vec3 uCenter;uniform vec3 uModelMin;uniform vec3 uModelRange;uniform vec2 uViewport;uniform float uScale;uniform float uYaw;uniform float uPitch;varying vec3 vNormal;void main(){vec3 p=(uModelMin+aPosition*uModelRange)-uCenter;float cy=cos(uYaw),sy=sin(uYaw);float xr=p.x*cy+p.z*sy;float zr=-p.x*sy+p.z*cy;float cp=cos(uPitch),sp=sin(uPitch);float yr=p.y*cp-zr*sp;float depth=p.y*sp+zr*cp;float nx=aNormal.x*cy+aNormal.z*sy;float nz=-aNormal.x*sy+aNormal.z*cy;vNormal=normalize(vec3(nx,aNormal.y*cp-nz*sp,aNormal.y*sp+nz*cp));gl_Position=vec4(2.0*xr*uScale/uViewport.x,2.0*(yr-depth*.12)*uScale/uViewport.y,-depth/420.0,1.0);}`);
      const fragmentShader=compileShader(gl,gl.FRAGMENT_SHADER,`precision mediump float;varying vec3 vNormal;void main(){vec3 normal=normalize(vNormal);if(!gl_FrontFacing)normal=-normal;float diffuse=max(dot(normal,normalize(vec3(-.38,.68,.62))),0.0);float light=.62+.38*diffuse;gl_FragColor=vec4(vec3(.98,.985,1.0)*light,1.0);}`);
      ...
      const packedPositions=new Uint16Array(binary,positionOffset,vertexCount*3),sourceIndices=new Uint32Array(binary,indexOffset,indexCount),expandedPositions=new Uint16Array(indexCount*3),vertexNormals=new Float32Array(vertexCount*3),expandedNormals=new Float32Array(indexCount*3);
      for(let index=0;index<indexCount;index+=3){const ia=sourceIndices[index],ib=sourceIndices[index+1],ic=sourceIndices[index+2],ax=packedPositions[ia*3]*glModelRange[0]/65535,ay=packedPositions[ia*3+1]*glModelRange[1]/65535,az=packedPositions[ia*3+2]*glModelRange[2]/65535,bx=packedPositions[ib*3]*glModelRange[0]/65535,by=packedPositions[ib*3+1]*glModelRange[1]/65535,bz=packedPositions[ib*3+2]*glModelRange[2]/65535,cx=packedPositions[ic*3]*glModelRange[0]/65535,cy=packedPositions[ic*3+1]*glModelRange[1]/65535,cz=packedPositions[ic*3+2]*glModelRange[2]/65535,abx=bx-ax,aby=by-ay,abz=bz-az,acx=cx-ax,acy=cy-ay,acz=cz-az,nx=aby*acz-abz*acy,ny=abz*acx-abx*acz,nz=abx*acy-aby*acx;for(const vertex of [ia,ib,ic]){vertexNormals[vertex*3]+=nx;vertexNormals[vertex*3+1]+=ny;vertexNormals[vertex*3+2]+=nz}}
      for(let vertex=0;vertex<vertexCount;vertex++){const offset=vertex*3,nx=vertexNormals[offset],ny=vertexNormals[offset+1],nz=vertexNormals[offset+2],length=Math.hypot(nx,ny,nz)||1;vertexNormals[offset]=nx/length;vertexNormals[offset+1]=ny/length;vertexNormals[offset+2]=nz/length}
      for(let index=0;index<indexCount;index++){const source=sourceIndices[index];expandedPositions[index*3]=packedPositions[source*3];expandedPositions[index*3+1]=packedPositions[source*3+1];expandedPositions[index*3+2]=packedPositions[source*3+2];expandedNormals[index*3]=vertexNormals[source*3];expandedNormals[index*3+1]=vertexNormals[source*3+1];expandedNormals[index*3+2]=vertexNormals[source*3+2]}
      ...
      const normalBuffer=gl.createBuffer();gl.bindBuffer(gl.ARRAY_BUFFER,normalBuffer);gl.bufferData(gl.ARRAY_BUFFER,expandedNormals,gl.STATIC_DRAW);const normalLocation=gl.getAttribLocation(glProgram,"aNormal");gl.enableVertexAttribArray(normalLocation);gl.vertexAttribPointer(normalLocation,3,gl.FLOAT,false,0,0);
      glIndexCount=indexCount;
  ...
  return <>
    ...
    <TraceViewer />
    <section className="film-section" id="film"><div className="film-heading"><span>02 · 영상 기록</span><h2>흔적을 넘어서 기억을 읽다</h2><p>기와에 남은 흔적을 디지털 기록과 해석으로 살펴봅니다.</p></div><div className="film-frame"><video controls playsInline preload="metadata" src="/feature-film.mp4">이 브라우저에서는 영상을 재생할 수 없습니다.</video></div></section>
  </>;
}
