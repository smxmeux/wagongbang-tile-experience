...
function ParticleStory() {
  ...
  useEffect(() => {
    ...
    const loadPolygonModel=async()=>{
      ...
      const vertexShader=compileShader(gl,gl.VERTEX_SHADER,`attribute vec3 aPosition;uniform vec3 uCenter;uniform vec3 uModelMin;uniform vec3 uModelRange;uniform vec2 uViewport;uniform float uScale;uniform float uYaw;uniform float uPitch;void main(){vec3 p=(uModelMin+aPosition*uModelRange)-uCenter;float cy=cos(uYaw),sy=sin(uYaw);float xr=p.x*cy+p.z*sy;float zr=-p.x*sy+p.z*cy;float cp=cos(uPitch),sp=sin(uPitch);float yr=p.y*cp-zr*sp;float depth=p.y*sp+zr*cp;gl_Position=vec4(2.0*xr*uScale/uViewport.x,2.0*(yr-depth*.12)*uScale/uViewport.y,-depth/420.0,1.0);}`);
      const fragmentShader=compileShader(gl,gl.FRAGMENT_SHADER,`precision mediump float;void main(){gl_FragColor=vec4(1.0,1.0,1.0,1.0);}`);
      ...
      const packedPositions=new Uint16Array(binary,positionOffset,vertexCount*3),sourceIndices=new Uint32Array(binary,indexOffset,indexCount),expandedPositions=new Uint16Array(indexCount*3);
      for(let index=0;index<indexCount;index++){const source=sourceIndices[index];expandedPositions[index*3]=packedPositions[source*3];expandedPositions[index*3+1]=packedPositions[source*3+1];expandedPositions[index*3+2]=packedPositions[source*3+2]}
