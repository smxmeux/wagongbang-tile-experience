...
function TraceViewer() {
  ...
  useEffect(()=>{
    ...
    const load=async()=>{
      ...
      landmarkTriangles=tileTraces.map(trace=>{const texturePoint=traceTexturePoints[trace.id]??[.5,.5],desiredU=texturePoint[0],desiredV=1-texturePoint[1];let bestTriangle=0,bestDistance=Infinity;for(let triangle=0;triangle<count;triangle+=3){const a=packedIndices[triangle],b=packedIndices[triangle+1],c=packedIndices[triangle+2],u=(packedUvs[a*2]+packedUvs[b*2]+packedUvs[c*2])/(3*65535),v=(packedUvs[a*2+1]+packedUvs[b*2+1]+packedUvs[c*2+1])/(3*65535),distance=(u-desiredU)**2+(v-desiredV)**2;if(distance<bestDistance){bestDistance=distance;bestTriangle=triangle}}return [packedIndices[bestTriangle],packedIndices[bestTriangle+1],packedIndices[bestTriangle+2]].map(index=>[0,1,2].map(axis=>modelMin[axis]+packedPositions[index*3+axis]/65535*modelRange[axis]))});
