...
function TraceViewer() {
  ...
  useEffect(()=>{
    ...
    let landmarkPoints:number[][]=[],center=[0,0,0];
    ...
    const load=async()=>{
      ...
      landmarkPoints=tileTraces.map(trace=>{const texturePoint=traceTexturePoints[trace.id]??[.5,.5],desiredU=texturePoint[0],desiredV=1-texturePoint[1];let bestTriangle=0,bestDistance=Infinity;for(let triangle=0;triangle<count;triangle+=3){const a=packedIndices[triangle],b=packedIndices[triangle+1],c=packedIndices[triangle+2],u=(packedUvs[a*2]+packedUvs[b*2]+packedUvs[c*2])/(3*65535),v=(packedUvs[a*2+1]+packedUvs[b*2+1]+packedUvs[c*2+1])/(3*65535),distance=(u-desiredU)**2+(v-desiredV)**2;if(distance<bestDistance){bestDistance=distance;bestTriangle=triangle}}const indices=[packedIndices[bestTriangle],packedIndices[bestTriangle+1],packedIndices[bestTriangle+2]];return [0,1,2].map(axis=>indices.reduce((sum,index)=>sum+modelMin[axis]+packedPositions[index*3+axis]/65535*modelRange[axis],0)/3)});
    ...
    const draw=()=>{if(!alive){return}const w=target.clientWidth,h=target.clientHeight,dpr=Math.min(devicePixelRatio,innerWidth<760?1.25:1.5);if(target.width!==w*dpr||target.height!==h*dpr){target.width=w*dpr;target.height=h*dpr;gl?.viewport(0,0,target.width,target.height)}if(!dragging){if(!userInteracted)yaw+=.0013;yaw+=yawVelocity;yawVelocity*=.95}const scale=Math.min(w/360,h/500)*1.45*zoomRef.current;if(gl&&program&&uniforms){gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);gl.useProgram(program);gl.uniform3f(uniforms.center,center[0],center[1],center[2]);gl.uniform3f(uniforms.modelMin,modelMin[0],modelMin[1],modelMin[2]);gl.uniform3f(uniforms.modelRange,modelRange[0],modelRange[1],modelRange[2]);gl.uniform2f(uniforms.viewport,w,h);gl.uniform1f(uniforms.scale,scale);gl.uniform1f(uniforms.yaw,yaw);gl.uniform1f(uniforms.pitch,pitch);gl.drawElements(gl.TRIANGLES,indexCount,gl.UNSIGNED_INT,0)}
      ...
      for(let i=0;i<landmarkPoints.length;i++){const marker=markerRefs.current[i],point=landmarkPoints[i];if(!marker||!point)continue;const x0=point[0]-center[0],y0=point[1]-center[1],z0=point[2]-center[2],rotX=y0,rotY=-x0,rotZ=z0,xr=rotX*Math.cos(yaw)+rotZ*Math.sin(yaw),zr=-rotX*Math.sin(yaw)+rotZ*Math.cos(yaw),yr=rotY*Math.cos(pitch)-zr*Math.sin(pitch),depth=rotY*Math.sin(pitch)+zr*Math.cos(pitch),px=w/2+xr*scale,py=h/2+(-yr+depth*.1)*scale,isInnerTrace=i<2,visible=!isSide&&isInnerTrace===viewingInner&&px>28&&px<w-28&&py>28&&py<h-28;marker.style.transform=`translate3d(${px}px,${py}px,0)`;marker.style.opacity=visible?"1":"0";marker.style.pointerEvents=visible?"auto":"none"}
      frame=requestAnimationFrame(draw)};
