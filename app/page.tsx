...
function TraceViewer() {
  ...
  const [tracePoints, setTracePoints] = useState<Record<string,[number,number]>>(traceTexturePoints);
  ...
  useEffect(()=>{
    const projectUrl=process.env.NEXT_PUBLIC_SUPABASE_URL,publishableKey=process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY;
    if(!projectUrl||!publishableKey)return;
    fetch(`${projectUrl}/rest/v1/trace_points?select=id,u,v`,{headers:{apikey:publishableKey}})
      .then(response=>response.ok?response.json():Promise.reject(new Error(`Supabase ${response.status}`)))
      .then((rows:{id:string;u:number;v:number}[])=>setTracePoints(current=>({...current,...Object.fromEntries(rows.map(row=>[row.id,[row.u,row.v]]))})))
  ...
  useEffect(()=>{
    ...
    let alive=true,frame=0,yaw=-.18,pitch=.08,yawVelocity=0,dragging=false,userInteracted=false,lastX=0,lastY=0,markerDraggingIndex=-1;
    ...
    const move=(event:globalThis.PointerEvent)=>{if(!dragging)return;const dx=event.clientX-lastX;yaw+=dx*.012;yawVelocity=dx*.0015;lastX=event.clientX;lastY=event.clientY};
    const up=()=>{dragging=false;yawVelocity=0};
    const projectPoint=(point:number[],scale:number,w:number,h:number)=>{const x0=point[0]-center[0],y0=point[1]-center[1],z0=point[2]-center[2],rotX=y0,rotY=-x0,rotZ=z0,xr=rotX*Math.cos(yaw)+rotZ*Math.sin(yaw),zr=-rotX*Math.sin(yaw)+rotZ*Math.cos(yaw),yr=rotY*Math.cos(pitch)-zr*Math.sin(pitch),depth=rotY*Math.sin(pitch)+zr*Math.cos(pitch);return{x:w/2+xr*scale,y:h/2+(-yr+depth*.1)*scale,depth}};
    const savePoint=async(index:number,u:number,v:number)=>{const projectUrl=process.env.NEXT_PUBLIC_SUPABASE_URL,publishableKey=process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY;if(!projectUrl||!publishableKey)return;const id=tileTraces[index].id;const response=await fetch(`${projectUrl}/rest/v1/trace_points?id=eq.${encodeURIComponent(id)}`,{method:"PATCH",headers:{apikey:publishableKey,Authorization:`Bearer ${publishableKey}`,"Content-Type":"application/json",Prefer:"return=minimal"},body:JSON.stringify({u,v,updated_at:new Date().toISOString()})});if(!response.ok)throw new Error(`Supabase ${response.status}`);setTracePoints(current=>({...current,[id]:[u,v]}))};
    ...
    const upMarker=(event:globalThis.PointerEvent)=>{if(markerDraggingIndex<0||!packedPositions||!packedUvs||!packedIndices)return;const index=markerDraggingIndex;markerDraggingIndex=-1;markerRefs.current[index]?.classList.remove("moving");const bounds=target.getBoundingClientRect(),pointerX=event.clientX-bounds.left,pointerY=event.clientY-bounds.top,w=target.clientWidth,h=target.clientHeight,scale=Math.min(w/360,h/500)*1.45*zoomRef.current;let minimumDistance=Infinity;const triangleProjection=(triangle:number)=>{const indices=[packedIndices![triangle],packedIndices![triangle+1],packedIndices![triangle+2]],points=indices.map(vertex=>[0,1,2].map(axis=>modelMin[axis]+packedPositions![vertex*3+axis]/65535*modelRange[axis])),projected=points.map(point=>projectPoint(point,scale,w,h));return{indices,x:projected.reduce((sum,point)=>sum+point.x,0)/3,y:projected.reduce((sum,point)=>sum+point.y,0)/3,depth:projected.reduce((sum,point)=>sum+point.depth,0)/3}};for(let triangle=0;triangle<triangleCount;triangle+=3){const point=triangleProjection(triangle),distance=(point.x-pointerX)**2+(point.y-pointerY)**2;if(distance<minimumDistance)minimumDistance=distance}let bestTriangle=0,bestDepth=-Infinity;for(let triangle=0;triangle<triangleCount;triangle+=3){const point=triangleProjection(triangle),distance=(point.x-pointerX)**2+(point.y-pointerY)**2;if(distance<=minimumDistance+196&&point.depth>bestDepth){bestDepth=point.depth;bestTriangle=triangle}}const vertices=[packedIndices[bestTriangle],packedIndices[bestTriangle+1],packedIndices[bestTriangle+2]],u=vertices.reduce((sum,vertex)=>sum+packedUvs![vertex*2]/65535,0)/3,v=vertices.reduce((sum,vertex)=>sum+packedUvs![vertex*2+1]/65535,0)/3;savePoint(index,u,v).catch(error=>console.warn("포인트 위치를 저장하지 못했습니다.",error))};
    const draw=()=>{if(!alive){return}const w=target.clientWidth,h=target.clientHeight,dpr=Math.min(devicePixelRatio,innerWidth<760?1.25:1.5);if(target.width!==w*dpr||target.height!==h*dpr){target.width=w*dpr;target.height=h*dpr;gl?.viewport(0,0,target.width,target.height)}if(!dragging){if(!userInteracted)yaw+=.0013;yaw+=yawVelocity;yawVelocity*=.95}const scale=Math.min(w/360,h/500)*1.45*zoomRef.current;if(gl&&program&&uniforms){gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);gl.useProgram(program);gl.activeTexture(gl.TEXTURE0);gl.bindTexture(gl.TEXTURE_2D,modelTexture);gl.uniform3f(uniforms.center,center[0],center[1],center[2]);gl.uniform3f(uniforms.modelMin,modelMin[0],modelMin[1],modelMin[2]);gl.uniform3f(uniforms.modelRange,modelRange[0],modelRange[1],modelRange[2]);gl.uniform2f(uniforms.viewport,w,h);gl.uniform1f(uniforms.scale,scale);gl.uniform1f(uniforms.yaw,yaw);gl.uniform1f(uniforms.pitch,pitch);gl.drawElements(gl.TRIANGLES,indexCount,gl.UNSIGNED_INT,0)}
      ...
      for(let i=0;i<landmarkTriangles.length;i++){const marker=markerRefs.current[i],triangle=landmarkTriangles[i];if(!marker||!triangle||i===markerDraggingIndex)continue;const projected=triangle.map(point=>projectPoint(point,scale,w,h)),surfaceX=projected.reduce((sum,point)=>sum+point.x,0)/3,surfaceY=projected.reduce((sum,point)=>sum+point.y,0)/3,px=Math.min(w-32,Math.max(32,surfaceX+(w/2-surfaceX)*.055)),py=Math.min(h-32,Math.max(32,surfaceY+(h/2-surfaceY)*.055)),isInnerTrace=i<2,visible=!isSide&&isInnerTrace===viewingInner;marker.style.transform=`translate3d(${px}px,${py}px,0)`;marker.style.opacity=visible?"1":"0";marker.style.pointerEvents=visible?"auto":"none"}
      frame=requestAnimationFrame(draw)};
    target.addEventListener("pointerdown",down);target.addEventListener("pointermove",move);target.addEventListener("pointerup",up);target.addEventListener("pointercancel",up);addEventListener("pointermove",moveMarker);addEventListener("pointerup",upMarker);draw();return()=>{alive=false;cancelAnimationFrame(frame);if(image)image.src="";target.removeEventListener("pointerdown",down);target.removeEventListener("pointermove",move);target.removeEventListener("pointerup",up);target.removeEventListener("pointercancel",up);removeEventListener("pointermove",moveMarker);removeEventListener("pointerup",upMarker);markerRefs.current.forEach((marker,index)=>marker?.removeEventListener("pointerdown",markerDownHandlers[index]))};
  },[tracePoints]);

  const changeZoom=(next:number)=>setZoom(Math.max(.72,Math.min(1.65,next)));
  return <section className="trace-viewer-section" id="traces"><div className="trace-viewer-heading"><span>02 · 제작 흔적 탐색</span><h2>표면에 남은<br/>일곱 개의 기록</h2><p>기와를 손가락으로 돌리고, 표시된 포인트를 눌러 장인의 손과 도구가 남긴 흔적을 살펴보세요. 내면과 외면의 포인트는 해당 면을 보고 있을 때만 나타납니다.</p><button className={`trace-edit-toggle ${editing?"active":""}`} onClick={()=>setEditing(value=>!value)}>{editing?"편집 완료":"포인트 위치 편집"}</button></div><div className={`trace-stage ${editing?"editing":""}`}><canvas ref={canvas} aria-label="좌우로 회전하고 확대할 수 있는 세로형 3D 기와 흔적 탐색기"/>{tileTraces.map((trace,index)=><button key={trace.id} ref={element=>{markerRefs.current[index]=element}} className="trace-marker trace-viewer-marker ready" onPointerDown={event=>event.stopPropagation()} onClick={()=>{if(!editing)setActiveTrace(trace)}} aria-label={editing?`${trace.name} 포인트 이동`:`${trace.name} 설명 열기`}><i/><span>{String(index+1).padStart(2,"0")}</span><b>{trace.name}</b></button>)}<div className="trace-surface-label" ref={surfaceLabelRef} data-surface="inner"><span>INNER · 내면</span><b>포목흔 · 합철흔</b></div><div className="trace-controls"><button onClick={()=>changeZoom(zoom-.12)} aria-label="기와 축소">−</button><button onClick={()=>changeZoom(1)} aria-label="확대 초기화">{Math.round(zoom*100)}%</button><button onClick={()=>changeZoom(zoom+.12)} aria-label="기와 확대">＋</button></div><p className="trace-gesture">{editing?"포인트를 원하는 흔적 위로 드래그하면 자동 저장됩니다":"좌우로 드래그하여 회전 · 버튼으로 확대"}</p></div>
