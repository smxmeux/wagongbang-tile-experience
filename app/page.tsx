...
function TraceViewer() {
  ...
  return <section className="trace-viewer-section" id="traces"><div className="trace-viewer-heading"><span>02 · 제작 흔적 탐색</span><h2>표면에 남은<br/>일곱 개의 기록</h2><p>기와를 손가락으로 돌리고, 표시된 포인트를 눌러 장인의 손과 도구가 남긴 흔적을 살펴보세요. 내면과 외면의 포인트는 해당 면을 보고 있을 때만 나타납니다.</p><button className={`trace-edit-toggle ${editing?"active":""}`} onClick={()=>{setEditing(value=>!value);setSaveStatus("idle")}}>{editing?"편집 완료":"포인트 위치 편집"}</button></div><div className={`trace-stage ${editing?"editing":""}`}><canvas ref={canvas} aria-label="좌우로 회전하고 확대할 수 있는 세로형 3D 기와 흔적 탐색기"/>{tileTraces.map((trace,index)=><button key={trace.id} ref={element=>{markerRefs.current[index]=element}} className="trace-marker trace-viewer-marker ready" onPointerDown={event=>event.stopPropagation()} onClick={()=>{if(!editing)setActiveTrace(trace)}} aria-label={editing?`${trace.name} 포인트 이동`:`${trace.name} 설명 열기`}><i/><span>{String(index+1).padStart(2,"0")}</span><b>{trace.name}</b></button>)}<div className="trace-surface-label" ref={surfaceLabelRef} data-surface="inner"><span>INNER · 내면</span><b>포목흔 · 합철흔 · 와도질</b></div><div className="trace-controls"><button onClick={()=>changeZoom(zoom-.12)} aria-label="기와 축소">−</button><button onClick={()=>changeZoom(1)} aria-label="확대 초기화">{Math.round(zoom*100)}%</button><button onClick={()=>changeZoom(zoom+.12)} aria-label="기와 확대">＋</button></div><p className={`trace-gesture save-${saveStatus}`}>{editing?(saveStatus==="saving"?"저장 중…":saveStatus==="saved"?"✓ 저장 완료":saveStatus==="error"?"저장 실패 · 잠시 후 다시 시도하세요":"포인트를 원하는 흔적 위로 드래그하면 자동 저장됩니다"):"좌우로 드래그하여 회전 · 버튼으로 확대"}</p></div>
