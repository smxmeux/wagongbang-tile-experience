...
function TraceViewer() {
  ...
  useEffect(()=>{
    ...
    let alive=true,frame=0,yaw=-.18,pitch=.08,yawVelocity=0,dragging=false,userInteracted=false,lastX=0,lastY=0,downX=0,downY=0,pointerMoved=false;
    let landmarkTriangles:number[][][]=[],markerScreenPositions:{x:number;y:number}[]=[],center=[0,0,0];
    ...
    let markerProgram:WebGLProgram|null=null,markerTexture:WebGLTexture|null=null,modelTexture:WebGLTexture|null=null;
    let markerUniforms:{point:WebGLUniformLocation|null;marker:WebGLUniformLocation|null;center:WebGLUniformLocation|null;viewport:WebGLUniformLocation|null;scale:WebGLUniformLocation|null;yaw:WebGLUniformLocation|null;pitch:WebGLUniformLocation|null;atlas:WebGLUniformLocation|null}|null=null;
    ...
    const load=async()=>{
      ...
      const packedPositions=new Uint16Array(binary,positionOffset,vertexCount*3),packedUvs=new Uint16Array(binary,uvOffset,vertexCount*2),packedIndices=new Uint32Array(binary,indexOffset,count);
      landmarkTriangles=tileTraces.map(trace=>{const desired=trace.anchor;let bestTriangle=0,bestDistance=Infinity;for(let triangle=0;triangle<count;triangle+=3){const indices=[packedIndices[triangle],packedIndices[triangle+1],packedIndices[triangle+2]],centroid=[0,1,2].map(axis=>indices.reduce((sum,index)=>sum+packedPositions[index*3+axis]/65535,0)/3),distance=centroid.reduce((sum,value,axis)=>sum+(value-desired[axis])**2,0);if(distance<bestDistance){bestDistance=distance;bestTriangle=triangle}}return [packedIndices[bestTriangle],packedIndices[bestTriangle+1],packedIndices[bestTriangle+2]].map(index=>[0,1,2].map(axis=>modelMin[axis]+packedPositions[index*3+axis]/65535*modelRange[axis]))});
      ...
      const markerVertex=compile(gl,gl.VERTEX_SHADER,`uniform vec3 uPoint;uniform vec3 uCenter;uniform vec2 uViewport;uniform float uScale;uniform float uYaw;uniform float uPitch;uniform float uMarker;varying float vMarker;void main(){vec3 p0=uPoint-uCenter;vec3 p=vec3(p0.y,-p0.x,p0.z);float cy=cos(uYaw),sy=sin(uYaw);float xr=p.x*cy+p.z*sy;float zr=-p.x*sy+p.z*cy;float cp=cos(uPitch),sp=sin(uPitch);float yr=p.y*cp-zr*sp;float depth=p.y*sp+zr*cp;gl_Position=vec4(2.0*xr*uScale/uViewport.x,2.0*(-yr+depth*.1)*uScale/uViewport.y,-depth/420.0-.0015,1.0);gl_PointSize=36.0;vMarker=uMarker;}`),markerFragment=compile(gl,gl.FRAGMENT_SHADER,`precision mediump float;uniform sampler2D uAtlas;varying float vMarker;void main(){vec2 uv=vec2((gl_PointCoord.x+vMarker)/7.0,1.0-gl_PointCoord.y);vec4 color=texture2D(uAtlas,uv);if(color.a<.08)discard;gl_FragColor=color;}`);
      markerProgram=gl.createProgram()!;gl.attachShader(markerProgram,markerVertex);gl.attachShader(markerProgram,markerFragment);gl.linkProgram(markerProgram);
      const atlas=document.createElement("canvas");atlas.width=224;atlas.height=32;const atlasContext=atlas.getContext("2d")!;atlasContext.font="700 11px Arial";atlasContext.textAlign="center";atlasContext.textBaseline="middle";for(let i=0;i<7;i++){const x=i*32+16;atlasContext.beginPath();atlasContext.arc(x,16,13,0,Math.PI*2);atlasContext.fillStyle="#050505";atlasContext.fill();atlasContext.lineWidth=2;atlasContext.strokeStyle="#f2f2ee";atlasContext.stroke();atlasContext.fillStyle="#ffffff";atlasContext.fillText(String(i+1).padStart(2,"0"),x,16)}markerTexture=gl.createTexture();gl.bindTexture(gl.TEXTURE_2D,markerTexture);gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA,gl.RGBA,gl.UNSIGNED_BYTE,atlas);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_S,gl.CLAMP_TO_EDGE);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_T,gl.CLAMP_TO_EDGE);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,gl.LINEAR);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MAG_FILTER,gl.LINEAR);
      markerUniforms={point:gl.getUniformLocation(markerProgram,"uPoint"),marker:gl.getUniformLocation(markerProgram,"uMarker"),center:gl.getUniformLocation(markerProgram,"uCenter"),viewport:gl.getUniformLocation(markerProgram,"uViewport"),scale:gl.getUniformLocation(markerProgram,"uScale"),yaw:gl.getUniformLocation(markerProgram,"uYaw"),pitch:gl.getUniformLocation(markerProgram,"uPitch"),atlas:gl.getUniformLocation(markerProgram,"uAtlas")};
    };
    load().catch(error=>console.error("Trace viewer initialization failed",error));
    const down=(event:globalThis.PointerEvent)=>{dragging=true;pointerMoved=false;userInteracted=true;downX=lastX=event.clientX;downY=lastY=event.clientY;yawVelocity=0;target.setPointerCapture(event.pointerId)};
    const move=(event:globalThis.PointerEvent)=>{if(!dragging)return;if(Math.hypot(event.clientX-downX,event.clientY-downY)>7)pointerMoved=true;const dx=event.clientX-lastX,dy=event.clientY-lastY;yaw+=dx*.012;pitch=Math.max(-.7,Math.min(.7,pitch+dy*.008));yawVelocity=dx*.0015;lastX=event.clientX;lastY=event.clientY};
    const up=(event:globalThis.PointerEvent)=>{if(dragging&&!pointerMoved){const bounds=target.getBoundingClientRect(),x=event.clientX-bounds.left,y=event.clientY-bounds.top;let closest=-1,distance=26;markerScreenPositions.forEach((point,index)=>{const next=Math.hypot(point.x-x,point.y-y);if(next<distance){closest=index;distance=next}});if(closest>=0)setActiveTrace(tileTraces[closest])}dragging=false;yawVelocity=0};
    const draw=()=>{if(!alive){return}const w=target.clientWidth,h=target.clientHeight,dpr=Math.min(devicePixelRatio,innerWidth<760?1.25:1.5);if(target.width!==w*dpr||target.height!==h*dpr){target.width=w*dpr;target.height=h*dpr;gl?.viewport(0,0,target.width,target.height)}if(!dragging){if(!userInteracted)yaw+=.0013;yaw+=yawVelocity;yawVelocity*=.95}const scale=Math.min(w/360,h/500)*1.45*zoomRef.current;if(gl&&program&&uniforms){gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);gl.useProgram(program);gl.activeTexture(gl.TEXTURE0);gl.bindTexture(gl.TEXTURE_2D,modelTexture);gl.uniform3f(uniforms.center,center[0],center[1],center[2]);gl.uniform3f(uniforms.modelMin,modelMin[0],modelMin[1],modelMin[2]);gl.uniform3f(uniforms.modelRange,modelRange[0],modelRange[1],modelRange[2]);gl.uniform2f(uniforms.viewport,w,h);gl.uniform1f(uniforms.scale,scale);gl.uniform1f(uniforms.yaw,yaw);gl.uniform1f(uniforms.pitch,pitch);gl.drawElements(gl.TRIANGLES,indexCount,gl.UNSIGNED_INT,0)}
      if(gl&&markerProgram&&markerUniforms&&markerTexture){gl.useProgram(markerProgram);gl.activeTexture(gl.TEXTURE0);gl.bindTexture(gl.TEXTURE_2D,markerTexture);gl.uniform1i(markerUniforms.atlas,0);gl.uniform3f(markerUniforms.center,center[0],center[1],center[2]);gl.uniform2f(markerUniforms.viewport,w,h);gl.uniform1f(markerUniforms.scale,scale);gl.uniform1f(markerUniforms.yaw,yaw);gl.uniform1f(markerUniforms.pitch,pitch);gl.disable(gl.DEPTH_TEST);gl.enable(gl.BLEND);gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA);gl.depthMask(false);for(let i=0;i<landmarkTriangles.length;i++){const triangle=landmarkTriangles[i],point=[0,1,2].map(axis=>triangle.reduce((sum,vertex)=>sum+vertex[axis],0)/3);gl.uniform3f(markerUniforms.point,point[0],point[1],point[2]);gl.uniform1f(markerUniforms.marker,i);gl.drawArrays(gl.POINTS,0,1)}gl.depthMask(true);gl.disable(gl.BLEND);gl.enable(gl.DEPTH_TEST)}
      const faceAmount=Math.cos(yaw),isSide=Math.abs(faceAmount)<.18,viewingInner=faceAmount>=0;
      if(surfaceLabelRef.current){Object.assign(surfaceLabelRef.current.style,{position:"absolute",zIndex:"7",left:innerWidth<760?"12px":"18px",top:innerWidth<760?"70px":"18px",display:"flex",flexDirection:"column",gap:"5px",maxWidth:innerWidth<760?"230px":"320px",padding:innerWidth<760?"9px 10px":"11px 13px",borderLeft:"2px solid #eee",background:"rgba(5,5,5,.8)",pointerEvents:"none"});surfaceLabelRef.current.dataset.surface=isSide?"side":viewingInner?"inner":"outer";surfaceLabelRef.current.innerHTML=isSide?"<span>측면</span><b>기와를 조금 더 돌려보세요</b>":viewingInner?"<span>INNER · 내면</span><b>포목흔 · 합철흔</b>":"<span>OUTER · 외면</span><b>와도질 · 외면부 · 지두흔 · 지문 · 인장</b>"}
      markerScreenPositions=[];for(let i=0;i<landmarkTriangles.length;i++){const marker=markerRefs.current[i],triangle=landmarkTriangles[i];if(!marker||!triangle)continue;const projected=triangle.map(point=>{const x0=point[0]-center[0],y0=point[1]-center[1],z0=point[2]-center[2],rotX=y0,rotY=-x0,rotZ=z0,xr=rotX*Math.cos(yaw)+rotZ*Math.sin(yaw),zr=-rotX*Math.sin(yaw)+rotZ*Math.cos(yaw),yr=rotY*Math.cos(pitch)-zr*Math.sin(pitch),depth=rotY*Math.sin(pitch)+zr*Math.cos(pitch);return{x:w/2+xr*scale,y:h/2+(-yr+depth*.1)*scale}}),px=projected.reduce((sum,point)=>sum+point.x,0)/3,py=projected.reduce((sum,point)=>sum+point.y,0)/3,visible=px>18&&px<w-18&&py>18&&py<h-18;markerScreenPositions[i]={x:px,y:py};marker.style.transform=`translate3d(${px}px,${py}px,0)`;marker.style.opacity=visible?"1":"0";marker.style.pointerEvents="none"}
      frame=requestAnimationFrame(draw)};
