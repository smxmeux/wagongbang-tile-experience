...
function TraceViewer() {
  ...
  useEffect(()=>{
    ...
    const savePoint=async(index:number,u:number,v:number)=>{const projectUrl=process.env.NEXT_PUBLIC_SUPABASE_URL,publishableKey=process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY;if(!projectUrl||!publishableKey)throw new Error("Supabase 환경 변수가 없습니다.");const id=tileTraces[index].id;const response=await fetch(`${projectUrl}/rest/v1/trace_points?id=eq.${encodeURIComponent(id)}`,{method:"PATCH",headers:{apikey:publishableKey,"Content-Type":"application/json",Prefer:"return=minimal"},body:JSON.stringify({u,v,updated_at:new Date().toISOString()})});if(!response.ok)throw new Error(`Supabase ${response.status}: ${await response.text()}`)};
